...
import FemaleProfilePopup from "../components/FemaleProfilePopup";
...
export default function HomeFeed() {
  ...
  const [currentFemaleProfile, setCurrentFemaleProfile] = useState(null);
  const [femalePopupQueue, setFemalePopupQueue] = useState([]);
  ...
  // Show female popups for first-time users
  useEffect(() => {
    const showFemalePopups = async () => {
      if (currentUser?.requiresFirstSubscription && femalePopupQueue.length === 0) {
        try {
          const response = await api.get('/users/random-females?limit=3');
          if (response.success && response.profiles.length > 0) {
            setFemalePopupQueue(response.profiles);
            
            // Show first popup after 2 seconds
            setTimeout(() => {
              setCurrentFemaleProfile(response.profiles[0]);
  ...
  const handlePopupClose = () => {
    ...
    if (popupShownCount < 3 && popupShownCount < femalePopupQueue.length) {
      setTimeout(() => {
  ...
  const handlePopupInteract = () => {
    setShowFemalePopup(false);
    // Redirect handled by FemaleProfilePopup component
  };
  ...
  // Handle direct message (females only - navigate to messages)
  const handleDirectMessage = async (profile) => {
    const isFemale = currentUser?.gender?.toLowerCase() === 'female';
    
    if (!isFemale) {
      showError("Please match with this user first to send messages.", "Match Required");
      return;
    }

    // Navigate to messages page with this user
    // Store the target user in sessionStorage for Messages page to pick up
  ...
  return (
    <div className="min-h-screen bg-sunset-gradient pb-0 lg:pb-0 lg:pr-64 overflow-x-hidden w-full">
      ...
      {/* Female Profile Popup for First-Time Users */}
      {showFemalePopup && currentFemaleProfile && (
        <FemaleProfilePopup
          profile={currentFemaleProfile}
          onClose={handlePopupClose}
          onInteract={handlePopupInteract}
        />
      )}
