...
const Disliked = () => {
  ...
  const fetchDislikedUsers = async () => {
    try {
      const response = await api.get('/interactions/disliked');
  ...
  const undoDislike = async (userId) => {
    if (!confirm('Undo this dislike? They may appear in your feed again.')) return;
    
    try {
      await api.delete(`/interactions/${userId}`);
      setDislikedUsers(prev => prev.filter(item => item.user._id !== userId));
    } catch (error) {
      ...
      <div className="bg-white/10 backdrop-blur-sm shadow-romantic">
        <div className="max-w-md mx-auto px-4 py-6">
          <div className="flex items-center space-x-4">
            ...
            />
            <div>
              <h1 className="text-2xl font-bold text-gray-900">People I Disliked</h1>
              <p className="text-gray-600 mt-1">{dislikedUsers.length} people</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-md mx-auto px-4 py-6">
        {dislikedUsers.length === 0 ? (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-2">No dislikes yet</h3>
            <p className="text-gray-600 mb-6">People you dislike will appear here. You can undo dislikes if you change your mind.</p>
            <button
              onClick={() => navigate('/')}
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-gray-400 text-xs">
                      Disliked on {new Date(item.dislikedAt).toLocaleDateString()}
                    </p>
                  </div>
                  
                  <button
                    onClick={() => undoDislike(item.user._id)}
                    className="text-blue-500 hover:text-blue-600 p-2 flex flex-col items-center"
                    title="Undo dislike"
                  >
